 ...
 * author: shardice
 * version: 3.2.0
 * description: Родной магазин Lampa без кастомной прокрутки
 */
...
(function () {
    ...
    Lampa.Lang.add({
        plugin_home_store_name: {
            ru: 'Дом плагинов',
            en: 'Plugin Home',
            uk: 'Дім плагінів'
        },
        plugin_home_store_descr: {
    ...
    function icon() {
        return '<svg viewBox="0 0 42 42" xmlns="http://www.w3.org/2000/svg">' +
            '<defs><linearGradient id="ph_store_icon_320" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#00ffd0"/><stop offset="1" stop-color="#2f80ff"/></linearGradient></defs>' +
            '<rect x="7" y="7" width="28" height="28" rx="8" fill="url(#ph_store_icon_320)"/>' +
    ...
    function css() {
        if ($('#plugin-home-store-native-style-320').length) return;

        $('body').append('<style id="plugin-home-store-native-style-320">' +
            '[data-component="' + COMPONENT + '"]{display:flex!important;align-items:center!important;gap:.85em!important;min-height:4.4em!important;}' +
    ...
    function addStore() {
        ...
        try {
            ...
            var field = $(
                '<div class="settings-param selector" data-component="' + COMPONENT + '">' +
                    ...
                    '<div class="settings-param__body">' +
                        '<div class="settings-param__name">' + Lampa.Lang.translate('plugin_home_store_name') + '</div>' +
                        '<div class="settings-param__descr">' + Lampa.Lang.translate('plugin_home_store_descr') + '</div>' +
                    '</div>' +
                '</div>'
            );
            ...
            if (after && after.length) after.after(field);
            else render.append(field);

            if (typeof main.update === 'function') main.update();
        } catch (e) {
            console.log('Plugin Home addStore error:', e);
        }
    }
    ...
    Lampa.Settings.listener.follow('open', function (e) {
        try {
            if (e.name == 'main') {
                e.body.find('[data-component="' + COMPONENT + '"]').off('hover:enter click').on('hover:enter click', openStore);
            }
        } catch (err) {
            console.log('Plugin Home settings open error:', err);
        }
    });
