...
(function () {
    ...
    Lampa.Lang.add({
        shardice_status_button: {
            ru: 'Статус',
            en: 'Status',
            uk: 'Статус'
        },
        shardice_statuses_menu: {
            ru: 'Мои статусы',
            en: 'My statuses',
            uk: 'Мої статуси'
        }
    ...
    function selectStatus(movie) {
        ...
        items.push({
            title: 'Убрать статус',
            subtitle: 'Удалить фильм из моих статусов',
            remove: true
        });
    ...
    function addFullButton(e) {
        try {
            ...
            if (body.find('[data-shardice-status-button]').length) return;

            var button = $('<div class="full-start__button selector" data-shardice-status-button="1"><span>' + Lampa.Lang.translate('shardice_status_button') + '</span></div>');
            ...
            console.log('Shardice Statuses error:', err);
        }
    }
    ...
    function addSettings() {
        ...
        if (render.find('[data-component="shardice_statuses"]').length) return;

        var field = $('<div class="settings-param selector" data-component="shardice_statuses">' +
            '<div class="settings-param__name">' + Lampa.Lang.translate('shardice_statuses_menu') + '</div>' +
            '<div class="settings-param__descr">Список фильмов и сериалов с вашими статусами</div>' +
        '</div>');

        var after = render.find('[data-component="shardice_store"], [data-component="more"]').first();
    ...
    function openList() {
        ...
        var items = Object.keys(data).map(function (key) {
            ...
            return {
                title: item.title,
                subtitle: item.status_title + (item.year ? ' • ' + item.year : '')
            };
        });
        ...
        if (!items.length) {
            items.push({
                title: 'Пока пусто',
                subtitle: 'Откройте фильм и нажмите кнопку «Статус»'
            });
        }
    ...
    Lampa.Settings.listener.follow('open', function (e) {
        if (e.name == 'main') {
            ...
            e.body.find('[data-component="shardice_statuses"]').off('hover:enter').on('hover:enter', openList);
        }
    });
